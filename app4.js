    ...
    for(var i = 0; i < 20; i++){
      ...
      $.getJSON(url, {
        ...
        .done(function(data) {
          ...
          // console.log(data);
          // console.log(data.data.results[26].name);
          for(var j = 0; j < 100; j++){
            if(data.data.results[j].id === undefined){
              console.log(data.data);
            }
            if(data.data.results[j].id){
              characterIDs.push(data.data.results[j].id);
              characterNames.push(data.data.results[j].name);
            }
          }
